...
static int arasan_gemac_mii_probe(struct net_device *dev)
{
	...
	/* linkmode_or(phydev->supported, PHY_GBIT_FEATURES, mask);
	linkmode_or(phydev->supported, SUPPORTED_FIBRE, mask);
	linkmode_or(phydev->supported, SUPPORTED_BNC, mask);
	linkmode_set_bit_array(ARASAN_GEMAC_FEATURES, ARRAY_SIZE(ARASAN_GEMAC_FEATURES), phydev->supported);
	linkmode_copy(phydev->supported, phydev->supported);*/
	/* phydev->supported &= ARASAN_GEMAC_FEATURES; */
	if (pd->phy_interface == PHY_INTERFACE_MODE_MII) {
		linkmode_clear_bit(ETHTOOL_LINK_MODE_1000baseT_Half_BIT, mask);
		linkmode_clear_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT, mask);
		/*linkmode_set_bit_array(~PHY_1000BT_FEATURES, ARRAY_SIZE(PHY_1000BT_FEATURES), phydev->supported);*/
...
static int arasan_gemac_probe(struct platform_device *pdev)
{
	...
	res = of_get_phy_mode(pdev->dev.of_node, &interface);
	if (res)
		goto err_disable_clocks;

	if (PHY_INTERFACE_MODE_MII != interface &&
	    PHY_INTERFACE_MODE_GMII != interface &&
	    PHY_INTERFACE_MODE_RGMII != interface) {
